...
vantage_point = get('https://api.ipify.org').text
...
class DNSDatabase:
    ...
    def insert_har(self, experiment, domain, cipher_level,network_type, recursive, dns_type, har, har_uuid, har_error, delays,webdelays):
        insert = ''' INSERT INTO {}
                     (uuid, experiment,vantage_point, insertion_time, domain,cipher_level,network_type, recursive, dns_type, har, error, delays,webdelays)
        ...
        if har:
            tup = (har_uuid, experiment,vantage_point, now, domain, cipher_level,network_type, recursive,
                   dns_type, Json(har), None, Json(delays),Json(webdelays))
        else:
            tup = (har_uuid, experiment,vantage_point, now, domain, cipher_level,network_type, recursive,
                   dns_type, None, har_error, Json(delays),Json(webdelays))
    ...
    def insert_dns(self, har_uuid, cipher_level,network_type, experiment,recursive,dns_type, all_dns_info):
        insert = '''INSERT INTO {}
                    (har_uuid, experiment, vantage_point,
                     insertion_time, domain,cipher_level,network_type,
                     recursive, dns_type,
        ...
        for domain in all_dns_info:
            ...
            tup = (har_uuid, experiment,vantage_point, now,
                   domain, cipher_level,network_type, recursive, dns_type,
                   response_size, response_time,
                   error)
        ...
        if experiments:
            ...
            cmd = '''
                SELECT har_uuid, experiment, domain, dns_type, recursive,
                       response_time, response_size, error, network_type
                FROM {}
                WHERE domain IN %s AND experiment IN %s
                '''.format(self.dns_table)
            rv = self._execute_command(cmd, (domains, experiments))
