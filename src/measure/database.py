...
class DNSDatabase:
    ...
    def insert_har(self, experiment, domain, recursive, dns_type, har, har_uuid, har_error,delays):
        insert = ''' INSERT INTO {}
                     (uuid, experiment,vantage_point, insertion_time, domain, recursive, dns_type, har, error,delays)
        ...
        if har:
            tup = (har_uuid, experiment,vantage_point, now, domain, recursive,
    ...
    def insert_dns(self, har_uuid, experiment,recursive,dns_type, all_dns_info):
        insert = '''INSERT INTO {}
                    (har_uuid, experiment, vantage_point,
                     insertion_time, domain,recursive, dns_type,
        ...
        if experiments:
            ...
            cmd = '''
                SELECT uuid, domain, experiment,
                       ...
                       jsonb_extract_path(har, 'pages', '0', 'pageTimings')->>'onLoad' as pageload
                FROM {}
                WHERE recursive  NOT IN ('google','quad9_security_edns','securedns_default','securedns_adblock') AND vantage_point='wired' AND domain IN %s 
                '''.format(self.har_table)
            rv = self._execute_command(cmd, (domains, ))
    ...
    def get_dns_timings(self, experiments=None):
        if experiments:
            experiments = tuple(experiments)
            cmd = '''
                SELECT har_uuid, vantage_point, experiment, domain, dns_type, recursive,
                       response_time, response_size, error
                FROM {}
                WHERE experiment IN %s
                '''.format(self.dns_table)
            ...
            cmd = '''
                 SELECT har_uuid,  vantage_point, experiment, domain, dns_type, recursive,
                       response_time, response_size, error
                FROM {} WHERE vantage_point='vodacom_4g'
                '''.format(self.dns_table)
            rv = self._execute_command(cmd)
