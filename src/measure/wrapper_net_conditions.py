...
from database_cipher_net import DNSDatabase
...
class Resolvers(enum.Enum):
    default = (None, None)
    cloudflare_nofilter = ('1.1.1.1', 'https://cloudflare-dns.com/dns-query')
    ...
    quad9_nofilter = ('9.9.9.10', 'https://dns10.quad9.net/dns-query')
    quad9_security = ('9.9.9.9', ' https://dns9.quad9.net/dns-query')
    quad9_security_edns = ('9.9.9.11', ' https://dns11.quad9.net/dns-query')
    cleanbrowsing_family_filter=('185.228.168.9','https://doh.cleanbrowsing.org/doh/family-filter')
    cleanbrowsing_adult_filter=('185.228.168.10','https://doh.cleanbrowsing.org/doh/adult-filter')
    cleanbrowsing_security_filter=('185.228.168.168','https://doh.cleanbrowsing.org/doh/security-filter')
    adguard_nofilter=('94.140.14.140','https://dns-unfiltered.adguard.com/dns-query')
    adguard_adblock=('94.140.14.14','https://dns.adguard.com/dns-query')
    adguard_family=('94.140.14.15','https://dns-family.adguard.com/dns-query')
...
def run(log, db, experiment, websites):
    network_options=['3G','4G','wifi', 'DSL', 'CN', 'default']
    dns_options = ['dns', 'doh', 'dot']
    cipher_levels = ['low', 'medium', 'high']
    recursive_options = ['default', 'cloudflare_nofilter', 'cloudflare_security', 'cloudflare_family',
                        'google','quad9_nofilter','quad9_security','quad9_security_edns',
                        'cleanbrowsing_adult_filter',
                        'cleanbrowsing_security_filter',
                        'cleanbrowsing_family_filter',
                        'adguard_nofilter','adguard_adblock','adguard_family'
                        ]


   # Shuffle the configurations for measurements we want to run
    options = list(itertools.product(recursive_options,dns_options,cipher_levels,network_options))
    random.shuffle(websites)
    for website in websites:
        random.shuffle(options)
        for recursive, dns_type,cipher_level,network_type in options:
            if recursive == 'default' and dns_type in ('doh', 'dot'):
                continue
            run_configuration(log, db, experiment, website,cipher_level,network_type,recursive, dns_type)


def run_configuration(log, db, experiment, website, cipher_level, network_type, recursive, dns_type):
    ...
    log.info('Collecting {} {} for {} using cipher level:{}'.format(recursive, dns_type, website,cipher_level))
    try:
        har, har_uuid, har_error, delays, all_dns_info,webdelays = \
            measure_and_collect_har(log, website, cipher_level,network_type,resolver_ip, resolver_uri, dns_type)

        # Insert HAR, ping times, and DNS timingsinto the db
        rv_har = db.insert_har(experiment, website, cipher_level,network_type, recursive,
                                dns_type,har, har_uuid, har_error, delays,webdelays)
        if not rv_har:
            msg = "Saved HAR for website {}, config: {}, {}, cipher_level:{}"
            log.info(msg.format(website, recursive, dns_type,cipher_level))
            if all_dns_info:
                rv_dns = db.insert_dns(har_uuid, cipher_level, network_type, experiment, recursive, dns_type, all_dns_info)
                if not rv_dns:
...
def measure_and_collect_har(log, website, cipher_level, network_type, resolver_ip, resolver_uri, dns_type):
    # Get a HAR for a website
    website = 'https://{0}'.format(website)
    if dns_type == 'dns':
        dns_opt = "--dns={0}".format(resolver_ip)
        cmd = ['docker', 'run', dns_opt, '--rm', 'dotdoh-cipher-levels',
                website, cipher_level, network_type, dns_type, 'n/a', 'n/a']
    else:
        cmd = ['docker', 'run', '--rm', 'dotdoh-cipher-levels',
                website,cipher_level,network_type, dns_type, resolver_ip, resolver_uri]
    ...
    return json_har, har_uuid, har_error, delays, all_dns_info,webdelays
