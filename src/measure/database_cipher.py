...
class DNSDatabase:
    ...
    def insert_har(self, experiment, domain, cipher, recursive, dns_type, har, har_uuid, har_error, delays,webdelays):
        insert = ''' INSERT INTO {}
                     (uuid, experiment,vantage_point, insertion_time, domain,cipher, recursive, dns_type, har, error, delays,webdelays)
        ...
        if har:
            tup = (har_uuid, experiment,vantage_point, now, domain, cipher, recursive,
                   dns_type, Json(har), None, Json(delays),Json(webdelays))
        else:
            tup = (har_uuid, experiment,vantage_point, now, domain, cipher, recursive,
                   dns_type, None, har_error, Json(delays),None,Json(webdelays))
    ...
    def insert_dns(self, har_uuid, cipher, experiment,recursive,dns_type, all_dns_info):
        insert = '''INSERT INTO {}
        ...
        for domain in all_dns_info:
            ...
            tup = (har_uuid, experiment,vantage_point, now,
                   domain, cipher, recursive, dns_type,
                   response_size, response_time,
                   error)
        ...
        if experiments:
            ...
            cmd = '''
                SELECT uuid, domain, experiment,
                       ...
                       jsonb_extract_path(har, 'pages', '0', 'pageTimings')->>'onLoad' as pageload
                FROM {}
                WHERE domain IN %s AND vantage_point IN ('168.253.229.13','41.13.196.36')
                '''.format(self.har_table)
            rv = self._execute_command(cmd, (domains, ))
