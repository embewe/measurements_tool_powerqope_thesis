...
#from ssl_utils import get_sslinfo
...
def measure_and_collect_har(log, website, resolver_ip, resolver_uri, dns_type):
    ...
    if dns_type == 'dns':
        ...
        cmd = ['docker', 'run', dns_opt, '--rm', 'tungs/firefox:filters',
                website, dns_type, 'n/a', 'n/a']
    else:
        cmd = ['docker', 'run', '--rm', 'tungs/firefox:filters',
    ...
    ''' try:
        ...
        sslinfo = get_sslinfo(website)
        
    except:
        log.error('Error getting SSL Information for {0}\n'.format(website))
        sslinfo = {}
    ...
    # Get TCP Timings
    try:
        tcptimings = tcp_timings(website)
