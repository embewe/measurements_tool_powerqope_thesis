...
def cf_pageloads(db, domains, xlimit, filename, experiments=None):
    ...
    for tup in pageloads:
        config = tup['recursive'] + '_' + tup['dns_type']+ '_' + tup['continent']
    ...
    dot_data = {
            ...
            "Quad9 DoT Europe": {"data": pageloads_per_config['quad9_dot_Europe'],
                               "color": "green", "linestyle": ":"}}
    plot_cdf(dot_data, xlimit, 0.3, "Page Load Time (seconds)", filename + "_dot_cdf.pdf")
    ...
    doh_data = {
            ...
            "Quad9 DoH Europe": {"data": pageloads_per_config['quad9_doh_Europe'],
                               "color": "green", "linestyle": ":"}}
    plot_cdf(doh_data, xlimit, 0.3, "Page Load Time (seconds)", filename + "_doh_cdf.pdf")
...
def plot_pageload_diffs(all_data, configs, shape, xlabel, filename, experiments=None):
    ...
    for i in range(shape[0]):
        ...
        for j in range(shape[1]):
            ...
            if i == 0:
                # i = row
                ax.set_title("{}\n{}".format(j + 1, prettify_label(c2)), size=4,rotation=0)
            ...
            if j != i:
                ...
                ax.set_xscale('symlog')
                ax.set_xticklabels(['-10', '-1', '0', '1', '10'],fontsize=4)
